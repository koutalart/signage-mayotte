...
async function initWeather() {
  try {
    ...
    const cur = {
      ...
      weathercode: m.hourly_codes ? m.hourly_codes[0] || 1 : 1
    };

    const cond = wmoToCondition(cur.weathercode);

    document.getElementById('meteo-big-icon').textContent = cond.icon;
    document.getElementById('meteo-big-temp').textContent = Math.round(cur.temp) + '°';
    document.getElementById('meteo-big-temp').style.color = cond.color;
    ...
    if (strip && m.hourly_temps) {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        div.innerHTML = `
          <div class="mf-hour">${String(idx%24).padStart(2,'0')}h</div>
          <div class="mf-icon">${fc.icon}</div>
          <div class="mf-temp" style="color:${fc.color}">${Math.round(m.hourly_temps[idx])}°</div>`;
...
async function initAll() {
  ...
  // Rotation des slides (garde ta fonction existante)
  buildRotation();
  showSlide(0);
  scheduleNext();
...
// Garde le reste de ton code (rotation, ticker RAF, radio grid, intro topics, RDV, etc.)
// Elles sont déjà bien implémentées dans ton fichier original.
